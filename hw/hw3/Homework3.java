...
public class Homework3 {
  ...
  public static void smoosh(int[] ints) {
    ...
    // blank lines or lines already present in this file.)
      int i;
      int[] store = new int[ints.length];
      store[0] = ints[0];
      int ptr1 = 1;
      int ptr2 = 0;
      while (ptr1 < ints.length){
	  if (ints[ptr1] == store[ptr2]){
	      ptr1++;
	  }else{
	      ptr2++;
	      store[ptr2] = ints[ptr1];
	  }
      }
      /*System.out.println(stringInts(store));*/
      for (i=ptr2+1;i<ints.length;i++){
	  store[i] = -1;
      }
